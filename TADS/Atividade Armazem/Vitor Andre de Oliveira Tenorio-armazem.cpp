/* 
    ...
    Solução de: Vitor André de Oliveira Tenório
...
{
    //cout << endl << "Chegou no primeiro construtor" << endl << endl;
    quantidadeDeItens = 0;
    capacidadeMaxima = capacidade;
    listaDeItens = new item[capacidadeMaxima];
}
armazem::armazem(armazem &copia)
{
    //cout << endl << "Chegou no segundo construtor" << endl << endl;
    quantidadeDeItens = copia.quantidadeDeItens;
    capacidadeMaxima = copia.capacidadeMaxima;
    listaDeItens = new item[capacidadeMaxima];
    copy(copia.listaDeItens, copia.listaDeItens + quantidadeDeItens, listaDeItens);
}
armazem::~armazem()
{
    delete[] listaDeItens;
...
{
    if (quantidadeDeItens == capacidadeMaxima)
...
{
    item *aux = new item[quantidadeDeItens];
    copy(listaDeItens, listaDeItens + quantidadeDeItens, aux);
    delete[] listaDeItens;

    capacidadeMaxima = ceil(capacidadeMaxima * 1.2);
    listaDeItens = new item[capacidadeMaxima];

    copy(aux, aux + quantidadeDeItens, listaDeItens);
    delete[] aux;
}
...
int main()
{
    ...
    while (opcaoDoMenu != 's')
    {
        ...
        case 'i':
            ...
            cin >> umItem.nomeDoProduto >> umItem.validadeEmDias >> umItem.quantidadeEmQuilos;
        ...
        case 'b':
            ...
            if (meuArmazem.buscarItem(nome, umItem))
                cout << umItem.nomeDoProduto << " "
                     ...
                     << umItem.quantidadeEmQuilos << endl
                     << endl;
            else
