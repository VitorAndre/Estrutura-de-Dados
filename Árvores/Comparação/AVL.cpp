...
public:
    noh(const unsigned &umDado) : elemento(umDado), esq(NULL), dir(NULL), altura(1) {}
    ~noh() {}
};
...
private:
    ...
    noh *insereAux(noh *umNoh, const unsigned &umDado); //feito
...
public:
    avl() { raiz = NULL; }
    ~avl();
    unsigned qtdNohsInsercao = 0;
    unsigned qtdNohsBusca = 0;
    void imprimir();

    // inserção e remoção são recursivos
    void insere(const unsigned &umDado);
    void remove(tipoChave chave);

    // busca retorna uma cópia do objeto armazenado
    unsigned busca(tipoChave chave);
...
void avl::insere(const unsigned &umDado)
{
    this->qtdNohsInsercao = 0;
    raiz = insereAux(raiz, umDado);
}

// inserção recursiva, devolve nó para atribuição de pai ou raiz
noh *avl::insereAux(noh *umNoh, const unsigned &umDado)
{
    this->qtdNohsInsercao++;
    if (umNoh == NULL)
    {
...
unsigned avl::busca(tipoChave chave)
{
    this->qtdNohsBusca = 0;
...
unsigned avl::percorreEmOrdemAux(noh *atual)
{
    if (atual != NULL)
    {
        ...
        if (atual->elemento.qtdadeFilmesApresentados >= maior and atual->elemento < ano)
        {
    ...
    do
    {
        try
        {
            ...
            case 'i': // Inserir recursivamente
                ...
                cout << "Quantidade de Nohs acessados na inserção: " << arvore.qtdNohsInsercao << endl;
                arvore.qtdNohsInsercao = 0;
                break;
            ...
            case 'b':                         // Buscar
                ...
                cout << "Quantidade de Nohs acessados na busca: " << arvore.qtdNohsBusca << endl;
                arvore.qtdNohsBusca = 0;
                break;
            ...
            case 'e': // Escrever tudo (em ordem)
                cout << endl
                     << arvore;
                break;
            case 'q': // Qtd de nohs acessados
                cout << "Quantidade de Nohs acessados na inserção: " << arvore.qtdNohsInsercao;
