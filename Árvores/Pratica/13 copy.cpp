...
private:
    ...
    //feito
    unsigned levantamentoAux(noh *umNoh, unsigned ptrAnoProcurado, unsigned ptrMaiorNumFilmesApres);
...
unsigned avl::levantamento()
{
    ...
    */
    if (raiz == NULL)
        ...
        return levantamentoAux(raiz, raiz->elemento.ano, raiz->elemento.qtdadeFilmesApresentados);
}
unsigned x;
unsigned y;
unsigned avl::levantamentoAux(noh *umNoh, unsigned ano, unsigned maior_qtd_filmes)
{

    if (umNoh != NULL)
    {
        // cout << "Ano:" << umNoh->elemento.ano << endl;

        if ((umNoh->elemento.qtdadeFilmesApresentados == maior_qtd_filmes and umNoh->elemento.ano < ano) or (umNoh->elemento.qtdadeFilmesApresentados > maior_qtd_filmes))
        {
            cout << "ANO: " << umNoh->elemento.ano << endl;
            //            x = umNoh->elemento.ano;
            cout << "Antes: " << ano << endl;
            return levantamentoAux(umNoh, umNoh->elemento.ano, umNoh->elemento.qtdadeFilmesApresentados);
        }
        else
        {
            x = levantamentoAux(umNoh->esq, ano, maior_qtd_filmes);
            y = levantamentoAux(umNoh->dir, ano, maior_qtd_filmes);
            //          x = ano;
        }
    }
    //cout << ano;
